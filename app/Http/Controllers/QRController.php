...
{   $nombre= Graduando::where('id_qr', $qr)->value('nombres', 'apellidos');
    $apellidos= Graduando::where('id_qr', $qr)->value('apellidos');
    $nombre = $nombre . ' ' .$apellidos;
      $rutaImagen = 'svg-qrs/codigo-qr-' . str_replace(' ', '-', strtolower($qr)) . '.svg';
    $grupo = str_replace('_', ' ', $grupo); // Nombre del grupo que deseas mostrar

    return view('viewqr1', ['rutaImagen' => $rutaImagen], ['nombre'=>$nombre]);
    ...
    public function storage(Request $request)
    {
        ...
        if ($informacion && $nombre_grupo === $informacion->nombre_grupo) {
            ...
            for ($i = 0; $i < $numero_qrs; $i++) {
                ...
                $graduando->numero_entradas =  $request->input('numero_entradas');
                $nombre_grupo = $request->input('nombre_grupo');
...
public function consultar_informacion_entrada(Request $request)
{
    ...
    public function saveInfo(){
        ...
        return redirect()->route('descargar-excel1')->with('status', 'Se limpiaron los datos exitosamente');
